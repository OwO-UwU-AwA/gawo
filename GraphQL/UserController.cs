...
public class UserController : GraphController
{
    ...
    private async Task<User> GetUserDb(string? username, string? id)
    {
        ...
        {
            query = await db.Query("SELECT * FROM Users WHERE username = type::string($username);",
                new Dictionary<string, object> { { "username", username! } }).ConfigureAwait(false);
        }
        else if (!id.IsNullOrEmpty())
        {
            var t = new Thing("Users", id!);
            query = await db.Query("SELECT * FROM Users WHERE id = type::thing($thing);",
                new Dictionary<string, object> { { "thing", t } }).ConfigureAwait(false);
        }
        else
        {
